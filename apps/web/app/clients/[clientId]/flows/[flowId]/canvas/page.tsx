    ...
    <div className="mx-auto max-w-2xl p-8">
      ...
      <div className="mt-4">
        <SamplePayloadForm
          ...
          ctaLabel="Continue"
          onSave={async (parsed) => {
            await api.saveFlowDefinition(clientId, flowId, { samplePayload: parsed });
            onSaved(parsed);
          }}
        />
      </div>
...
function CanvasInner({
  ...
  useEffect(() => {
    async function load() {
      const [flow, rules] = await Promise.all([
        api.getFlowDefinition(clientId, flowId).catch(() => null),
        api.listRules(clientId, flowId).catch(() => []),
      ]);
  ...
  const handleDeleteRuleNode = useCallback(
    (ruleNodeId: string, opts?: { skipCanvasRemoval?: boolean }) => {
      ...
      if (target?.type === 'rule') {
        api.deleteRule(clientId, flowId, (target.data as RuleNodeData).rule.ruleId).catch(() => {
  ...
  async function handleSave(): Promise<boolean> {
    ...
    try {
      ...
      await api.saveFlowDefinition(clientId, flowId, flowDefinition);
      ...
      for (const ruleNode of ruleNodes) {
        ...
        await api.saveRule(clientId, flowId, updatedRule.ruleId, updatedRule);
      }
  ...
  async function handleTestNow() {
    ...
    setTestResult(null);
    try {
      const { executionId } = await api.testFlow(clientId, flowId, samplePayload);
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
          const detail = await api.getExecutionDetail(clientId, flowId, executionId);
          ...
          {testResult && (
            ...
            >
              ...
              {testResult.status && testResult.executionId && (
                <Link
                  href={`/clients/${clientId}/flows/${flowId}/executions/${testResult.executionId}`}
